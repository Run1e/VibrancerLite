CreateBigGUI() {
	...
	Big.Font("s11")
	Big.GameLV := new Big.ListView(Big, "x" 0 " y" 0 " w" HALF_WIDTH - 1 " h" LV_HEIGHT " -HDR -Multi +LV0x4000 -E0x200 AltSubmit -TabStop", "name|path", Big.GameListViewAction.Bind(Big))
	
	Big.Font("s10")
	
	Big.GameDeleteHWND := Big.Add("Button", "x1 y" LV_HEIGHT + 1 " w" Round(HALF_WIDTH/5*2) - 2 " h" BUTTON_HEIGHT - 1,, Big.GameDelete.Bind(Big))
	...
	Big.Add("Text", "x" HALF_WIDTH - 1 " y" 0 " w1 h" LV_HEIGHT " 0x08") ; skille
	Big.Add("Text", "x" HALF_WIDTH " y" LV_HEIGHT/2 + 8 " w" HALF_WIDTH " h1 0x08") ; skille
	
	Big.Font("s11")
	
	Big.Margin(6, 4) ; nicerino margerino
	Big.VibrancySliderTextHWND := Big.Add("Text", "x" HALF_WIDTH " y" 8 " w" HALF_WIDTH " Center")
	Big.VibrancySliderHWND := Big.Add("Slider", "x" HALF_WIDTH + 12 " yp+25 w" HALF_WIDTH - 48 " Range50-100 ToolTip Center",, Big.GamesSlider.Bind(Big))
	Big.VibrancyIndicatorHWND := Big.Add("Text", "x" HALF_WIDTH * 2 - 36 " yp+10 w30 Center")
	
	Big.WinKeyBlockHWND := Big.Add("CheckBox", "x" HALF_WIDTH + 16 " yp+44 w122 h40",, Big.GamesWinBlock.Bind(Big)) ; egt 70
	Big.AltTabBlockHWND := Big.Add("CheckBox", "x" HALF_WIDTH/2*3 + 6 " yp w125 h40",, Big.GamesAltTabBlock.Bind(Big))
	
	Big.VibrancyScreenHWND := Big.Add("Text", "x" HALF_WIDTH + 6 " y" 150 " W" HALF_WIDTH - 12 " Center")
	...
	if (MonitorCount = 1) {
		Big.PrimarySelectedHWND := Big.Add("Text", "x" HALF_WIDTH + 1 " y" LV_HEIGHT*3/4 " w" HALF_WIDTH - 12 " Center", "Primary Selected!")
	...
	Big.SetText(Big.GameAddHWND, "Add Program")
	Big.SetText(Big.GameDeleteHWND, "Remove")
	Big.SetText(Big.WinKeyBlockHWND, "Block Win Key")
	Big.SetText(Big.AltTabBlockHWND, "Block Alt Tab")
	Big.SetText(Big.VibrancySliderTextHWND, "NVIDIA Vibrance Boost")
	Big.SetText(Big.VibrancyScreenHWND, "Select Screen(s):")
	Big.SetText(Big.PrimaryScreenHWND, "Primary screen selected!")
	
	for Index, Button in [Big.GameDeleteHWND, Big.GameAddHWND]
	...
	ImageButton.Create(Big.GamesTabHWND, NORMAL*)
}